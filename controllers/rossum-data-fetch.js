...
const makeUrl = (id, date_start, date_end) =>
    `https://elis.rossum.ai/api/v1/queues/${id}/export?format=json&exported_at_after=${toISOString(date_start)}&exported_at_before=${toISOString(date_end)}&page_size=100&page=1`;
...
const fetchXMLs = (date_start, date_end, token) => {
    const filePath = path.join(__dirname, '../rossum_data', 'data_' + date_start.getTime() + '_' + date_end.getTime() + '.xml');
